...
Send data.txt
  $ curl -v -H "Accept: application/text" -H "Content-type: application/text" -c cookies.txt -X POST --data @data.txt 'http://localhost:3000/recordings/upload'
...
In case of accelerometers you can use also one of the following:
