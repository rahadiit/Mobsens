...
public class PrettyPrint {
	...
	public static void print(ArrayList<Location> locations) {
		...
		if (locations.isEmpty()) {
			...
			double distance = GPS.distanceKMSumLoc(locations);
			System.out.println("locations: " + locations.size());
			System.out.println("Distance: " + distance);
			System.out.println("duration "
					+ Time.duration(locations.get(0),
							locations.get(locations.size() - 1), "HH:mm:ss"));
			System.out.println("avg speed: " + Calc.getAverageSpeed(locations));
			System.out.println("start: "
					+ DateFormatUtils.format(new Date(time1), "HH:mm:ss"));
			System.out.println("stop: "
					+ DateFormatUtils.format(new Date(time2), "HH:mm:ss"));
			...
			if (distance > steps / 1000) {
				...
				for (int step : Calc.indicesBySteps(locations, steps)) {
					...
					System.out.println("Distance: "
							+ GPS.distanceKMSumLoc(locations, start, step));
					System.out.println("duration "
							+ Time.duration(locations.get(start),
									locations.get(step), "HH:mm:ss"));
					System.out.println("avg speed: "
							+ Calc.getAverageSpeed(locations, start, step));
