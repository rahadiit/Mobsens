...
//		if (locationCSV!=null&& annotationCSV != null && acceleroCSV != null) {
			...
			for(Location location:locations){
				System.out.println(location.getSpeed()+" Sensor vs Calc "+location.getSpeedCalc());
			}
			
			for(Location location:locations){
				System.out.println(location.getDistanceCalc()+" SensorDist vs CalcDist "+location.getDistanceCalc2());
			}
			
			for(Location location:locations){
				System.out.println(location.getDistanceSumCalc()+" SensorDistSum vs CalcDistSum "+location.getDistanceSumCalc2());
			}
